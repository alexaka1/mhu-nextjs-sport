...
export default function ResultsContent({
    ...
    if (result.url == null) {
      return (
        <Fragment key={result.key}>
          <Heading />
          <p className={`prose text-balance p-2 text-primary-800 dark:text-primary-200`}>Valami hiba történt.</p>
        </Fragment>
      );
    }
    switch (result.type) {
            ...
            <ResultsTable file={result.url} canEdit={canEdit} title={result.result} fileKey={result.key} year={year}>
              ...
              >
                <iframe
                  ...
                  allow={`fullscreen`}
                ></iframe>
                <p className="prose text-balance p-0.5 text-sm text-primary-800 dark:text-primary-200">
            ...
            <ResultsTable file={result.url} canEdit={canEdit} title={result.result} fileKey={result.key} year={year}>
              <div className={`sm:mx-auto`}>
                <ZoomWrapper>
                  ...
                  <img id={result.key} alt={result.url} src={result.url} className={`size-auto`} />
                </ZoomWrapper>
              </div>
            </ResultsTable>
          </Fragment>
  ...
  return tables.length > 0 ?
      tables
    : <p className={`prose mx-auto p-4 text-primary-800 dark:text-primary-200`}>Nincs eredmény az adott sportágban.</p>;
}
